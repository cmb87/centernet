...
class Datapipe:
   ...
   # ============================
    ...
    def _gaussianLabel(self, img, boxes, labels, jsonfile):
        ...
        # Concat results
        y = tf.concat((hm, wh, pdelta, idx), axis=-1)
...
def postprocess(ylabel, pool_size=3, K=50):
    ...
    B, H, W, K = ylabel.shape
    C = K - 5

    # [B,H,W,C], [B,H,W,2], [B,H,W,2], [B,H,W,1]
    hm, wh, pdelta, inds = tf.split(ylabel, [C, 2, 2, 1], axis=-1)
...
if __name__ == "__main__":
    ...
    for x in g.take(4):
      ...
      #  print(tf.reduce_min(x[1][...,5:], axis=[1,2]))
        ...
        img = x[0][0,...].numpy()
        hm = cv2.resize(x[1][0,...,:nc].numpy(),(ih,iw))
        ...
        axs[0].imshow(x[1][0,...,:nc], vmin=0, vmax=1.0,  interpolation='none', aspect='equal')
        axs[1].imshow(x[1][0,...,nc+1], vmin=0, vmax=0.05,  interpolation='none', aspect='equal')
        axs[2].imshow(tf.reduce_sum(keep[0,...,:nc],axis=-1)*x[1][0,...,nc], vmin=0, vmax=0.05,  interpolation='none', aspect='equal')
